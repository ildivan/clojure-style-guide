...
=== Nomi idiomatici [[idiomatic-names]]
 ...
 ** `ns` - namespacefootnote:[Tecnicamente questo oscurerà la macro `ns`, ma è estremamente improbabile che tu ne abbia mai bisogno nel corpo di una funzione.]
* nelle macro:
 ** `expr` - un espressione
...
=== Nuova riga facoltativa dopo il nome della funzione[[optional-new-line-after-fn-name]]
...
=== Indentazione con arità multipla [[multiple-arity-indentation]]
...
=== Ordine delle arità multiple [[multiple-arity-order]]

Ordina le arità di una funzione dal minor numero al maggior numero di argomenti. Il caso comune di funzioni con arità multipla è che alcuni argomenti K specificano completamente il comportamento della funzione, e che le arità N < K applicano parzialmente l'arità K, e
le aritò N > K forniscono una piegatura dell'arità K su varargs.
...
=== Lunghezza delle funzioni [[function-length]]
